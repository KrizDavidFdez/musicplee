...
const lyricsCache = new Map<string, { lyrics: any[]; duration?: number; timestamp: number }>();
const CACHE_TTL_MS = 24 * 60 * 60 * 1000; // 24 hours

// Helper to normalize strings for clean comparison
...
const MALUMA_BORRO_CASSETTE_LRC = `[id: ngidzo64]
[ar: Maluma]
...
const NICKY_JAM_EL_AMANTE_LRC = `[id: ngid3ja5]
[ar: Nicky Jam]
[al: Fénix]
[ti: El Amante]
[length: 03:40]
...
function isBorroCassette(title: string, artist: string, query: string): boolean {
  const norm = (s: string) =>
    (s || '')
      .toLowerCase()
      .normalize('NFD')
  ...
  const isArtistMatch = normArtist.includes('maluma') || !normArtist;

  const isQueryMatch =
    (normQ.includes('borro') || normQ.includes('cassette') || normQ.includes('casette') || normQ.includes('casete')) &&
    (normQ.includes('maluma') || isArtistMatch);

  return (isTitleMatch && isArtistMatch) || isQueryMatch;
}

function isElAmante(title: string, artist: string, query: string): boolean {
  const norm = (s: string) =>
    (s || '')
      .toLowerCase()
      .normalize('NFD')
  ...
  const isTitleMatch = normTitle.includes('amante');
  const isArtistMatch = normArtist.includes('nicky') || normArtist.includes('jam') || !normArtist;

  const isQueryMatch =
    (normQ.includes('amante') && (normQ.includes('nicky') || normQ.includes('jam'))) ||
    (normTitle.includes('amante') && (normTitle.includes('nicky') || isArtistMatch));

  return (isTitleMatch && isArtistMatch) || isQueryMatch;
}

const NICKY_JAM_HASTA_EL_AMANECER_LRC = `[id: ngid1spm]
[ar: Nicky Jam]
[al: Fénix]
...
function isHastaElAmanecer(title: string, artist: string, query: string): boolean {
  const norm = (s: string) =>
    (s || '')
      .toLowerCase()
      .normalize('NFD')
  ...
  // Strictly exclude remix versions
  if (normTitle.includes('remix') || normQ.includes('remix')) return false;

  const isTitleMatch =
    normTitle.includes('hasta el amanecer') ||
    (normTitle.includes('amanecer') && !normTitle.includes('borro'));
  const isArtistMatch = normArtist.includes('nicky') || normArtist.includes('jam') || !normArtist;

  const isQueryMatch =
    normQ.includes('hasta el amanecer') ||
    (normQ.includes('amanecer') && (normQ.includes('nicky') || normQ.includes('jam')));

  return (isTitleMatch && isArtistMatch) || isQueryMatch;
}

const THE_MARIAS_LEJOS_DE_TI_LRC = `[id: ngir6im4]
[ar: The Marías]
[al: Submarine]
[ti: Lejos de Ti]
[length: 03:00]
...
function isLejosDeTi(title: string, artist: string, query: string): boolean {
  const norm = (s: string) =>
    (s || '')
      .toLowerCase()
      .normalize('NFD')
  ...
  const isTitleMatch = normTitle.includes('lejos de ti') || (normTitle.includes('lejos') && normTitle.includes('ti'));
  const isArtistMatch = normArtist.includes('marias') || normArtist.includes('maria') || !normArtist;

  const isQueryMatch =
    normQ.includes('lejos de ti') ||
    ((normQ.includes('lejos') && normQ.includes('ti')) && (normQ.includes('marias') || normQ.includes('maria') || isArtistMatch));

  return (isTitleMatch && isArtistMatch) || isQueryMatch;
}

function isAquelNapZzZz(title: string, artist: string, query: string): boolean {
  const norm = (s: string) =>
    (s || '')
      .toLowerCase()
      .normalize('NFD')
  ...
  const isArtistMatch = normArtist.includes('rauw') || normArtist.includes('alejandro') || !normArtist;

  const isQueryMatch =
    (normQ.includes('aquel nap') || normQ.includes('aquel napp') || normQ.includes('nappz') || normQ.includes('nap z') || normQ.includes('ngirwyqe')) &&
    (normQ.includes('rauw') || normQ.includes('alejandro') || isArtistMatch);

  return (isTitleMatch && isArtistMatch) || isQueryMatch;
}

function isLagunas(title: string, artist: string, query: string): boolean {
  const norm = (s: string) =>
    (s || '')
      .toLowerCase()
      .normalize('NFD')
  ...
  const isQueryMatch =
    normQ.includes('lagunas') ||
    (normQ.includes('peso pluma') && normQ.includes('lagunas')) ||
    normQ.includes('ngirhank');

  return (isTitleMatch && isArtistMatch) || isQueryMatch;
}

function isCorazonFrio(title: string, artist: string, query: string): boolean {
  const norm = (s: string) =>
    (s || '')
      .toLowerCase()
      .normalize('NFD')
  ...
  const isQueryMatch =
    normQ.includes('corazon frio') ||
    (normQ.includes('jasiel') && normQ.includes('corazon')) ||
    (normQ.includes('dannylux') && normQ.includes('corazon')) ||
    normQ.includes('nkxibsu4');
...
function isComoOlvidar(title: string, artist: string, query: string): boolean {
  const norm = (s: string) =>
    (s || '')
      .toLowerCase()
      .normalize('NFD')
  ...
  const isQueryMatch =
    normQ.includes('como olvidar') ||
    (normQ.includes('olvidar') && (normQ.includes('rels') || isArtistMatch));

  return (isTitleMatch && isArtistMatch) || isQueryMatch;
}

const FORBIDDEN_VARIANTS = ['remix', 'sped up', 'slowed', 'nightcore', 'reverb', 'acoustic', 'live', 'cover', 'tribute', 'instrumental', 'karaoke'];
...
function parseLrc(lrcText: string): { time: number; text: string }[] {
  ...
  // Extract optional [offset: +/-ms] tag
  let offsetSec = 0;
  ...
  for (const line of lines) {
    ...
    if (!trimmed) continue;
    if (/^\[(ar|ti|al|by|offset|length|re|ve|hash):/i.test(trimmed)) continue;

    const matches = [...trimmed.matchAll(timeRegex)];
    if (matches.length > 0) {
      const rawText = trimmed.replace(timeRegex, '').replace(/<[^>]*>/g, '').trim();
      ...
      // Skip empty lines to prevent accidental instrumental cuts while singing
      if (!text) continue;

      for (const match of matches) {
        const minutes = parseInt(match[1], 10);
        const seconds = parseFloat(match[2]);
        const time = Math.max(0, minutes * 60 + seconds + offsetSec);
        parsedLyrics.push({
          time,
          text,
        });
  ...
  const cleanLyrics: { time: number; text: string }[] = [];
...
export default async function handler(req: Request | any, res: Response | any) {
  ...
  // Check verified enhanced tracks FIRST before cache
  let syncedTrack: any = null;

  if (isBorroCassette(title, artist, rawQuery)) {
    syncedTrack = {
      syncedLyrics: MALUMA_BORRO_CASSETTE_LRC,
      duration: 207,
    };
  } else if (isElAmante(title, artist, rawQuery)) {
    syncedTrack = {
      syncedLyrics: NICKY_JAM_EL_AMANTE_LRC,
      duration: 220,
    };
  } else if (isHastaElAmanecer(title, artist, rawQuery)) {
    syncedTrack = {
      syncedLyrics: NICKY_JAM_HASTA_EL_AMANECER_LRC,
      duration: 199,
    };
  } else if (isLejosDeTi(title, artist, rawQuery)) {
    syncedTrack = {
      syncedLyrics: THE_MARIAS_LEJOS_DE_TI_LRC,
      duration: 180,
    };
  } else if (isAquelNapZzZz(title, artist, rawQuery)) {
    syncedTrack = {
      syncedLyrics: RAUW_ALEJANDRO_AQUEL_NAP_ZZZZ_LRC,
      duration: 295,
    };
  } else if (isLagunas(title, artist, rawQuery)) {
    syncedTrack = {
      syncedLyrics: PESO_PLUMA_LAGUNAS_LRC,
      duration: 231,
    };
  } else if (isCorazonFrio(title, artist, rawQuery)) {
    syncedTrack = {
      syncedLyrics: JASIEL_NUNEZ_CORAZON_FRIO_LRC,
      duration: 263,
    };
  } else if (isComoOlvidar(title, artist, rawQuery)) {
    syncedTrack = {
      syncedLyrics: RELS_B_COMO_OLVIDAR_LRC,
      duration: 108,
    };
  }

  if (syncedTrack) {
    const parsed = parseLrc(syncedTrack.syncedLyrics);
    return res.json({
      lyrics: parsed,
      duration: syncedTrack.duration || targetDuration,
      rawLrc: syncedTrack.syncedLyrics,
    });
  }

  const queryKey = cleanString(`${artist} ${title}`);
  const cached = lyricsCache.get(queryKey);
  if (cached && Date.now() - cached.timestamp < CACHE_TTL_MS) {
    return res.json({ lyrics: cached.lyrics, duration: cached.duration });
  }

  const wantsRemix = title.toLowerCase().includes('remix') || rawQuery.toLowerCase().includes('remix');

  try {

    // 1. Try exact match from LRCLIB get endpoint first
    if (!syncedTrack && title && artist) {
      const getUrl = `https://lrclib.net/api/get?track_name=${encodeURIComponent(title)}&artist_name=${encodeURIComponent(artist)}${targetDuration ? `&duration=${Math.round(targetDuration)}` : ''}`;
      try {
        ...
        if (getRes.ok) {
          ...
          if (getData && getData.syncedLyrics) {
            syncedTrack = getData;
          }
        }
      } catch {}
    ...
    // 2. If not found via exact get, search with LRCLIB search endpoint
    if (!syncedTrack) {
      const searchParam = artist && title ? `${artist} ${title}` : (rawQuery || title);
      const searchUrl = `https://lrclib.net/api/search?q=${encodeURIComponent(searchParam)}`;
      const controller = new AbortController();
      ...
      if (searchRes.ok) {
        ...
        if (Array.isArray(searchData) && searchData.length > 0) {
          // Filter tracks with synced lyrics
          const syncedCandidates = searchData.filter((t: any) => t.syncedLyrics);

          // Find track matching title WITHOUT remix/sped-up noise unless requested
          const bestMatch = syncedCandidates.find((t: any) => {
            const trackName = (t.trackName || '').toLowerCase();
            const artistName = (t.artistName || '').toLowerCase();

            if (!wantsRemix) {
              const hasForbidden = FORBIDDEN_VARIANTS.some((v) => trackName.includes(v));
              if (hasForbidden) return false;
            }

            if (artist && !artistName.includes(cleanString(artist)) && !cleanString(artist).includes(artistName)) {
              return false;
            }
          ...
          syncedTrack = bestMatch || syncedCandidates[0];
        }
      }
    }

    if (syncedTrack && syncedTrack.syncedLyrics) {
      const cleanLyrics = parseLrc(syncedTrack.syncedLyrics);
      const trackDuration = typeof syncedTrack.duration === 'number' && syncedTrack.duration > 0 ? syncedTrack.duration : undefined;

      lyricsCache.set(queryKey, { lyrics: cleanLyrics, duration: trackDuration, timestamp: Date.now() });
      return res.json({ lyrics: cleanLyrics, duration: trackDuration, rawLrc: syncedTrack.syncedLyrics });
    }
